...
2. Use `mcp_agentmail_reply_to_message` to send a reply
3. **MANDATORY: Always CC the inbox owner** — every outgoing email, no exceptions, even with `replyAll=true`.
4. **MANDATORY: Always include the FULL agent signature** — Courier New, `#1f2937`, 12px, no separator, in both `html` and `text`.
